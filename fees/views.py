...
def add_fee(request):
    ...
    if request.method == 'POST':
        ...
        if not Fee.objects.filter(department_id=department, fee_category=fee_category, semester_id=semester).exists():
            Fee.objects.create(fees=fee, department_id=department, last_date=payment_date,
                               fee_category=fee_category, semester_id=semester)
        else:
            messages.error(
                request, "Fee alreday added. If you are looking for editing data please go with edit option")
...
def student_fee(request):
    if request.method == 'GET':
        ...
        fees = Fee.objects.filter(
            department_id=student.user.department_id, fee_category=student.quota)
        for fee in fees:
            if fee.semester.order <= current_semester.order:
                fee_list.append(fee)
        payed_fee_list = []
        payed_fees = FeePaid.objects.filter(
            department_id=student.user.department_id, student_id=request.user.id)
        print(payed_fees)
...
def payment(request, payment_id):
    ...
    if request.method == 'POST':
        ...
        FeePaid.objects.create(student_id=request.user.id, semester_id=semester_id,
                               department_id=department_id, payed_date=date, fee_category=fee_category, fees=fees)
...
def sms(request):
    ...
    if request.method == 'POST':
        ...
        zerosms.sms(phno=request.user.username,
                    passwd=request.user.password, receivernum=phone, message=msg)
